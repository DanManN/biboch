...
inline int cbb::score(int player)
{
	...
	if (numBits(pb ^ ob) <= 12 && numBits(cb.k) > numBits(pb ^ ob) / 2) {
		...
		score += (player ? -1 : 1) * PRTY_MED *
		         (3 * numBits((cb.w ^ (cb.w & cb.k)) & 0x000F0000) -
		          3 * numBits((cb.b ^ (cb.b & cb.k)) & 0x0000F000) +
		          4 * numBits((cb.w ^ (cb.w & cb.k)) & 0x00F00000) -
		          4 * numBits((cb.b ^ (cb.b & cb.k)) & 0x00000F00) +
		          5 * numBits((cb.w ^ (cb.w & cb.k)) & 0x0F000000) -
		          5 * numBits((cb.b ^ (cb.b & cb.k)) & 0x000000F0) +
		          7 * numBits((cb.w ^ (cb.w & cb.k)) & 0x0000000F) -
		          7 * numBits((cb.b ^ (cb.b & cb.k)) & 0xF0000000));
...
{
	int score, bscore, i, nlmm1 = 0, tpick = 0, maxd = 0;
	...
	if (nlm == 0) { // computer player lost
		...
		while (++maxd < STACK_SIZE) { // iterative deepening
			...
			for (i = nlmm1; i >= 0; i--) { // start search at each legal move
				if (duration_cast<milliseconds>(system_clock::now() - start)
				            .count() > timeLimit / 3)
					goto BREAK;
				stack[0] = lms[i];
